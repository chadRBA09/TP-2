...
{ 
    ...
    void insertionSort(Jogador* jogador, int tamanho)
{
    for (int i = 1; i < tamanho; i++) 
    {
        Jogador temp = jogador[i];
        int j = i - 1;
        while (j >= 0 && fraseParaInteiro(jogador[j].nome) > fraseParaInteiro(temp.nome)) 
        {
            jogador[j + 1] = jogador[j];
            j--;
        }
        jogador[j + 1] = temp; // insere o jogador na posição correta
    }
}
...
int main () 
{
    ...
    while (strcmp(nome,"FIM") != 0)
    {
        ...
        while (esq <= dir)
        {
            ...
            if(strcmp(busca[meio].nome,nome) == 0) // se o nome do jogador for igual ao nome buscado
            {
               achou = true;
               esq = cont;
            }
            ...
            {
               if((fraseParaInteiro(busca[meio].nome) - fraseParaInteiro(nome) ) < 0) // se o nome do jogador for maior que o nome buscado (alfabeticamente)
               {
                   esq = meio + 1;
               }
               else // se o nome do jogador for menor que o nome buscado (alfabeticamente)
               {
                   dir = meio - 1;
               }
            }
